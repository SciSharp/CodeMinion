...
namespace Regen.Compiler.Digest {
    ...
    public class Interpreter : IEvaluator {
        ...
        public ParsedCode Interpret(Dictionary<string, object> variables = null) {
            return Interpret(RegenCode, variables);
        }

        public ParsedCode Interpret(string code, Dictionary<string, object> variables = null) {
            ...
            if (walker.Count == 0) {
                ...
                return new ParsedCode() {OriginalCode = code, Output = output, Variables = Context.Variables.ToDictionary(kv => kv.Key, kv => kv.Value)};
            }
            ...
            return new ParsedCode() {OriginalCode = code, Output = output, Variables = variables};
        }
